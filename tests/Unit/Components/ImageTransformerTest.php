...
class ImageTransformerTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testTransformImageWithAmpersands()
    {
        $attributesFilters = [
            'src' => ['h' => 100, 'w' => 100],
            'srcset' => [
        ...
        $imageRenderer
            ...
            ->with(
                $this->callback(
                    function ($domElement) {
                        return ($domElement instanceof \DOMElement)
                            && ('a & b' === $domElement->getAttribute('alt'))
                            && ('a & b' === $domElement->getAttribute('title'))
                            && ('foo' === $domElement->getAttribute('class'))
                            && ('trans1' === $domElement->getAttribute('src'))
                            && ('trans2' === $domElement->getAttribute('srcset'))
                            && ('trans3' === $domElement->getAttribute('sizes'))
            ...
            ->will($this->returnValue('<img alt="a & b" title="a &amp; b" class="foo" src="trans1" srcset="trans2" sizes="trans4" data-srcset="trans3" other="trans5 1x, trans6 1x">'));

        $imageTransformer = new ImageTransformer($attributeGenerator, $imageRenderer);

        $this->assertEquals(
            '<img alt="a & b" title="a &amp; b" class="foo" src="trans1" srcset="trans2" sizes="trans4" data-srcset="trans3" other="trans5 1x, trans6 1x">',
            $imageTransformer->transformImage('<img alt="a & b" title="a &amp; b" src="src.png" class="foo" other="http://mysite.com/extra1.png 1x, /extra2.png 1x">', $attributesFilters)
        );
    }
}
